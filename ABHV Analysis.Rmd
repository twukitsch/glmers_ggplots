...
plot(density(residuals(models$eth$aversive$overall)), 
     main="", xlab="", frame= FALSE)
# Add normal distribution to the residual plot for comparison to check assumption of normality
EavMR.m = mean(residuals(models$eth$aversive$overall)) # get the residual mean
EavMR.std = sqrt(var(residuals(models$eth$aversive$overall))) # get the st dev
curve(dnorm(x, mean=EavMR.m, sd=EavMR.std), col="darkblue", lwd=2, add=TRUE, yaxt="n") # Generate normal curve
remove(EavMR.m,EavMR.std) # Declutter workspace
```
...
```{r Aversive Responses: Overall Model Planned Comparisons}
  ...
  compars$eth$avers$overall$condition <- emmeans(models$eth$aversive$overall, ~ Condition) # Perform comparison of Condition with emmeans
...
```{r }
  ...
  ## Ethanol Group Only: MAC & ROC with Day 1 drinking (MR1) ####
